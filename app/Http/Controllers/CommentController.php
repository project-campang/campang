...
class CommentController extends Controller
{
    ...
    public function CommentGet(Request $request)
    {
        ...
        // 사용자가 작성한 댓글 중 deleted_at이 null인 데이터만 가져옵니다.
        $RankData = Comment::select(
                            'comments.id', 
                            'comments.comment', 
                            'comments.created_at', 
                            'comments.user_id', 
                        ...
                        ->leftJoin('camps', 'comments.camp_id', '=', 'camps.id') // camps 테이블과 조인
                        ->where('comments.user_id', '=', $userId) // 현재 로그인한 사용자의 댓글만 가져옴
                        ->whereNull('comments.deleted_at')
                        ->orderBy('comments.created_at', 'DESC')
                        ->get();
    
        // 사용자의 닉네임을 각 댓글에 추가합니다.
